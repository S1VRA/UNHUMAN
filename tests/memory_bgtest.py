...
# -*- coding: utf-8 -*-
"""
Memory measurement during --bgtest (real rembg inference).
"""
...
def run_bgtest_memory(exe_path):
    print(f"Running --bgtest on {exe_path}...")
    
    # Start the process
    proc = subprocess.Popen([exe_path, "--bgtest", os.path.dirname(exe_path)])
    ...
    # Read the report
    report_path = os.path.join(os.path.dirname(exe_path), "bgtest_report.json")
    if os.path.exists(report_path):
        with open(report_path, "r") as f:
            report = json.load(f)
        print(f"\nBGTest result: {'PASS' if report.get('pass') else 'FAIL'}")
